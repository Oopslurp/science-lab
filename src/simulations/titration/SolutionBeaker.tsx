...
export default function SolutionBeaker({ pH, fill, ariaLabel }: SolutionBeakerProps) {
  const color = pHColor(pH);
  const topY = 30 + (1 - fill) * 90; // surface du liquide (y croissant vers le bas)
  const bottomY = 130;
  const clipId = useId(); // id unique : évite toute collision si plusieurs béchers

  return (
    <svg viewBox="0 0 100 150" className="h-40 w-auto" role="img" aria-label={ariaLabel}>
